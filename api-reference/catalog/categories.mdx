...
<RequestExample>
```bash cURL
curl -X GET https://api.pixelbyte.app/v1/catalog/categories
```

```python Python
import requests

response = requests.get("https://api.pixelbyte.app/v1/catalog/categories")
...
const response = await fetch("https://api.pixelbyte.app/v1/catalog/categories");
