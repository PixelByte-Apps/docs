...
description: "Generate text to video outputs using Alibaba+'s Wan 2.6 model via PixelByte API"
---

Generate text to video outputs with Alibaba+'s Wan 2.6 model. Submitted as an asynchronous job and delivered via webhook or polling.
...
<ParamField body="model" type="string" required placeholder="alibaba-plus/wan-2.6/text-to-video">
  Model slug. Use `alibaba-plus/wan-2.6/text-to-video` for text to video generation.
</ParamField>
...
curl -X POST https://api.muvi.video/v1/jobs/submit \
  ...
  -d '{
    "model": "alibaba-plus/wan-2.6/text-to-video",
    "input": {
