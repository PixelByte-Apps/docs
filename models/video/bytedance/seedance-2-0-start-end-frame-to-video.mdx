---
title: "Seedance 2.0 — Start-End Frame to Video"
api: "POST /v1/jobs/submit"
description: "Generate a smooth video transition between a start frame and an end frame with ByteDance's Seedance 2.0 model"
---

Generate a smooth video transition between two frames with ByteDance's Seedance 2.0. Provide a start image, an end image, and a prompt describing the motion in between.
...
<ParamField body="model" type="string" required placeholder="bytedance/seedance-2.0/start-end-frame-to-video">
  Model slug. Use `bytedance/seedance-2.0/start-end-frame-to-video` for start-end frame to video generation.
</ParamField>
...
curl -X POST https://api.muvi.video/v1/jobs/submit \
  ...
  -d '{
    "model": "bytedance/seedance-2.0/start-end-frame-to-video",
    "input": {
