...
description: "Generate motion control using Kling 3.0 Pro via PixelByte API"
---

Transfer the motion from a reference video to a still image using Kling 3.0 Pro, producing a character-controlled animated video.
...
<ParamField body="model" type="string" required placeholder="kling-plus/kling-3.0-pro/motion-control">
  Model slug. Use `kling-plus/kling-3.0-pro/motion-control` for motion control generation.
</ParamField>
...
<Tip>
**Default cost:** Flat = **50,000 micro-cents ($0.05)**
</Tip>
...
curl -X POST https://api.muvi.video/v1/jobs/submit \
  ...
  -d '{
    "model": "kling-plus/kling-3.0-pro/motion-control",
    "input": {
