---
title: "Kling O1 Standard — Image to Video"
api: "POST /v1/jobs/submit"
description: "Generate videos with the Kling O1 Standard model using image to video."
---

Animate a still image into a high-quality video with the Kling O1 Standard model. Provide an `image_url` and an optional `prompt` to guide motion.
...
<ParamField body="model" type="string" required placeholder="kling-plus/kling-o1-standard/image-to-video">
  Model slug. Use `kling-plus/kling-o1-standard/image-to-video` for image to video generation.
</ParamField>
...
curl -X POST https://api.muvi.video/v1/jobs/submit \
  ...
  -d '{
    "model": "kling-plus/kling-o1-standard/image-to-video",
    "input": {
