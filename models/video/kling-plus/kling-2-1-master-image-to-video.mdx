...
description: "Generate image to video using Kling 2.1 Master via PixelByte API"
---

Animate a static image into a video using a text prompt with Kling 2.1 Master. Supports multiple durations and prompt-guidance controls.
...
<ParamField body="model" type="string" required placeholder="kling-plus/kling-2.1-master/image-to-video">
  Model slug. Use `kling-plus/kling-2.1-master/image-to-video` for image to video generation.
</ParamField>
...
curl -X POST https://api.muvi.video/v1/jobs/submit \
  ...
  -d '{
    "model": "kling-plus/kling-2.1-master/image-to-video",
    "input": {
